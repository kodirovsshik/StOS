...
SECTION .bss
bss:
	.vbe_modes_ptr resw 1
	.vbe_modes_count resw 1
	.vbe_modes_count_reported resw 1
...
.vbe_get_modes:
	...
	xor eax, eax
	mov ax, [bss.vbe_modes_count_reported]
	call put32u
	mov si, rodata.str_vbe_modes1
	call puts

	xor eax, eax
	mov ax, [bss.vbe_modes_count]
	call put32u
...
.bad:
	stc
.ret:
	lea sp, [esp + 256] ;to prevent modifying flags
	pop es
	popad
	ret
...
vbe_print_video_modes:
	mov si, [bss.vbe_modes_ptr]
	mov cx, [bss.vbe_modes_count]
	test cx, cx
.l:
	jz .ret
	lodsw
...
.l:
	...
	cmp ax, 0xFFFF
	je .le
	inc word [es:bss.vbe_modes_count_reported]
	call vbe_check_mode
	jc .l
	stosw
	inc word [es:bss.vbe_modes_count]
	jmp .l
.le:
